...
def step_index_from_hhmm(hhmm: str) -> int:
...
def step_indices_for_period(period_name: str) -> range:
    """Inclusive start, exclusive end — matches scene-cognition period bucketing."""
    window = DEFAULT_PERIOD_WINDOWS.get(period_name)
    if not window:
        return range(0)
    return range(step_index_from_hhmm(window[0]), step_index_from_hhmm(window[1]))


def prepare_turn_flow_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Drop direction-aggregate rows (turn_dir_no=0) and dedupe per step + movement."""
    explicit_rows = [row for row in rows if int(to_float(row.get("turn_dir_no"))) != 0]
    candidates = explicit_rows or list(rows)
    ...
    for row in candidates:
        ...
        key = (
            ...
            int(to_float(row.get("turn_dir_no"))),
        )
        current = seen.get(key)
        if current is None or to_float(row.get("turn_flow_total")) > to_float(current.get("turn_flow_total")):
            seen[key] = row
    if seen:
...
def turn_flow_daily_total(rows: list[dict[str, Any]]) -> float:
    """Sum 5-minute samples into daily volume (turn_flow_total is hourly-equivalent per slice)."""
    prepared = prepare_turn_flow_rows(rows)
    return round(sum(to_float(row.get("turn_flow_total")) / 12.0 for row in prepared), 2)


def avg_by_lane(rows: list[dict[str, Any]], field: str = "volume") -> dict[str, float]:
    """Average lane-level 5-minute volume counts, keyed by lane_id."""
    buckets: dict[str, list[float]] = {}
    for row in rows:
        lane_key = str(row.get("lane_id") or f"{row.get('link_id')}_{row.get('lane_no')}")
        raw = row.get(field)
        if raw is None and field == "volume":
            raw = row.get("vehicle_count")
        if raw is None:
            continue
        buckets.setdefault(lane_key, []).append(to_float(raw))
    return {key: round(sum(vals) / len(vals), 2) for key, vals in buckets.items() if vals}


def lane_flow_daily_total(rows: list[dict[str, Any]]) -> float:
    """Sum lane-level 5-minute volume counts into a daily total."""
    return round(
        sum(to_float(row.get("volume") or row.get("vehicle_count")) for row in rows),
        2,
    )
