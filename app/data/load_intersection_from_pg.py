...
DIR8_TRAVEL_LABELS = {k: v.replace("进口", "") for k, v in DIR8_ENTRY.items()}
DIR8_APPROACH_LABELS = DIR8_TRAVEL_LABELS

TURN_DIR_LABELS = {0: "方向聚合", 1: "左转", 2: "直行", 3: "右转", 4: "掉头"}
FLOW_CORRELATE_DAY_LABELS = {1: "周一", 2: "周二", 3: "周三", 4: "周四", 5: "周五", 6: "周六", 7: "周日"}
DIR8_LABELS = DIR8_TRAVEL_LABELS
FLOW_TYPE_TURN_LABELS = {1: "直", 2: "左", 3: "右", 4: "掉"}

# PG 宽表 turn_move 标准编码（dwd_tfc_rltn_wide_inter_ft_lane）
TURN_MOVE_STRAIGHT_CODES = frozenset({"11", "40"})
TURN_MOVE_LEFT_CODES = frozenset({"12", "21", "23", "24", "32"})
TURN_MOVE_RIGHT_CODES = frozenset({"13", "22", "23", "24", "34"})
TURN_MOVE_UTURN_CODES = frozenset({"31", "32", "33", "34"})
...
def _query_signal_lane_mapping(schema: str, inter_id: str) -> list[dict[str, Any]]:
    sql = f"""
        ...
        WHERE inter_id = :inter_id
          AND is_deleted = 0
        ORDER BY plan_no NULLS LAST, stage_no, signal_atom
    """
    return _read_pg(sql, {"inter_id": inter_id}, limit=5000)
...
def _load_config_module():
    from app.data import intersection_load_config as module
...
def _load_metrics_logic_module():
    ...
    spec = importlib.util.spec_from_file_location(
        "traffic_metrics_logic",
        scripts_dir / "traffic_metrics_logic.py",
    )
    if spec is None or spec.loader is None:
        raise RuntimeError("无法加载 app/data/traffic_metrics_logic.py")
...
def _filter_rows_by_step_range(
    ...
    if step_index is None:
        return rows
    end = step_index_end if step_index_end is not None else step_index
    lo, hi = min(step_index, end), max(step_index, end)
    filtered = [
        row
        for row in rows
        if row.get("step_index") is not None and lo <= int(row["step_index"]) <= hi
    ]
    if filtered:
        return filtered
    if not rows:
    ...
    target = (lo + hi) // 2
    nearest = min(
        (int(r["step_index"]) for r in rows if r.get("step_index") is not None),
        key=lambda idx: abs(idx - target),
        default=target,
    )
    return [row for row in rows if row.get("step_index") is not None and int(row["step_index"]) == nearest]
...
def _assemble_task_from_raw(
    ...
    eval_rows = _filter_rows_by_step_range(raw.get("evaluation") or [], step_index, step_index_end)
    sat_rows = _filter_rows_by_step_range(raw.get("turn_saturation") or [], step_index, step_index_end)
    flow_rows = _filter_rows_by_step_range(raw.get("turn_flow") or [], step_index, step_index_end)
    util_rows = _filter_rows_by_step_range(raw.get("green_utilization") or [], step_index, step_index_end)
    perf_rows = _filter_rows_by_step_range(raw.get("turn_perf") or [], step_index, step_index_end)
    lane_capacity_rows = _filter_rows_by_step_range(raw.get("lane_capacity") or [], step_index, step_index_end)
    lane_flow_rows = _filter_rows_by_step_range(raw.get("lane_flow") or [], step_index, step_index_end)
    ...
    if step_index is not None:
        ...
        context["step_index_end"] = step_index_end
        context["time_window_label"] = f"{_time_label(step_index)}-{_time_label(step_index_end or step_index)}"
...
def load_intersection_from_pg(
    ...
    """
    if time_range and step_index is None:
...
def _schedule_referenced_plan_nos(schema: str, inter_id: str) -> list[str]:
    """Collect plan_no values referenced by day-plan schedule periods."""
    plan_nos: set[str] = set()
    for row in _query_schedule_cfg(schema, inter_id):
        if not isinstance(row, dict):
            continue
        for key in ("period_plan_no", "plan_no"):
            value = row.get(key)
            if value not in (None, ""):
                plan_nos.add(str(value))
                break
    return sorted(plan_nos, key=lambda item: (_as_float(item), str(item)))


def _query_active_plan(schema: str, inter_id: str) -> list[dict[str, Any]]:
    """Load plan stage rows for all schedule-referenced plans (fallback: entire intersection)."""
    schedule_plans = _schedule_referenced_plan_nos(schema, inter_id)
    plan_filter = ""
    params: dict[str, Any] = {"inter_id": inter_id}
    if schedule_plans:
        placeholders = ", ".join(f":plan_{idx}" for idx in range(len(schedule_plans)))
        plan_filter = f" AND p.plan_no IN ({placeholders})"
        for idx, plan_no in enumerate(schedule_plans):
            params[f"plan_{idx}"] = plan_no
    ...
    """
    try:
        rows = _read_pg(extended_sql, params, limit=5000)
        if rows or schedule_plans:
            return rows
    except Exception:
        pass
...
def _aggregate_metrics(
    ...
    lane_flow_rows: list[dict[str, Any]] | None = None,
) -> dict[str, Any]:
    evaluation = (
        max(
            eval_rows,
    ...
    prepared_flow_rows = _TML.prepare_turn_flow_rows(flow_rows)
    movement_volume = _avg_by_movement(prepared_flow_rows, "turn_flow_total")
    movement_saturation = _max_by_movement(sat_rows, "turn_saturation")
    volume_hourly_avg = round(sum(movement_volume.values()), 2)
    volume_daily_total = _TML.turn_flow_daily_total(flow_rows)
    volume = volume_hourly_avg
    lane_volume = _TML.avg_by_lane(lane_flow_rows or [])
    lane_volume_daily_total = _TML.lane_flow_daily_total(lane_flow_rows or [])
    lane_count = max(
        (int(_as_float(row.get("lane_count"))) for row in prepared_flow_rows if row.get("lane_count") is not None),
    ...
    return {
        "volume": volume,
        "volume_hourly_avg": volume_hourly_avg,
        "volume_daily_total": volume_daily_total,
        "volume_unit": "pcu/h_avg",
...
def _movement_key(row: dict[str, Any]) -> str:
    link_id = str(row.get("link_id") or row.get("f_dir_8") or "").strip()
    ...
    *,
    ...
    for row in ordered_plan_rows:
        ...
        if not stage_no or stage_no in seen_stages:
            continue
        seen_stages.add(stage_no)
        green = int(_as_float(row.get("green_sec")))
        yellow = int(_as_float(row.get("yellow_sec") or 3))
        all_red = int(_as_float(row.get("all_red_sec") or 2))
    ...
    return {
        ...
        "max_cycle_s": int(cycle + 30) if cycle else 150,
...
def _build_context(
    ...
    return {
        ...
        "data_quality": {
            ...
            "has_lane_flow": bool(metrics.get("lane_volume")),
            "has_dws_coverage": bool(metrics.get("movement_volume") or metrics.get("saturation")),
        },
