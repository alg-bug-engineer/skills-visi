...
用户自然语言
    ↓
[首轮意图] LLM 二分类 + 规则兜底
    ├─ corridor_scan → 干线 NLU（corridor + 时段）→ 扫描排名 → 地图全量标注 → 用户选型
    │       ↓
    │   注入单点 NLU（路口+时段已有）→ 只追问方向 → 进入下方单点流水线
...
## 2. 干线扫描（2026-06-29 新增）

### 2.0 与单点诊断的分工

| 维度 | 干线扫描 | 单点诊断 |
|------|----------|----------|
| 触发 | 问一条路上哪些/哪个最堵 | 已指明具体路口或选型后 |
| NLU 字段 | `corridor` + `time_period` | `intersection` + `time_period` + `directions` |
| 追问 | 缺时段；**不问路口名** | 路口 → 时段 → 方向 |
| 输出 | 全量路口地图 + Top3 文字 | 渠化/证据/治理建议 |

实现索引：`intent_classifier_service.py`、`corridor_scan_service.py`、`corridor_pick_resolver.py`  
完整说明：`docs/plans/2026-06-27-干线扫描与路口发现.md`
...
## 8. 进口道流量溯源（2026-06-30 geom 重构）

过饱和或用户明示进口/转向时，编排器在原因诊断后调用 `UpstreamGovernanceTraceService`：

```
用户进口/转向（如西左转）
    ↓
UpstreamTopologyService：link 邻接 + geom 折线（主）
    ↓
flow_correlate 白名单内取流量占比（辅）
    ↓
单链 build_tree（≤5 跳）→ 治理落点
    ↓
map_action: upstream_tree（storyboard 本地播放）
```

要点：

- **单链**：同一 `corridor_dir8` 递归，不再多 arm 分叉。
- **禁飞线**：`edge.path` 来自 `hop_path`（`dim_link_info.geom`）；前端 `resolveEdgePath` 无两点 fallback。
- **触发**：`turn_saturation ≥ 0.90` 或 NLU `directions`；「西左转」只溯西进口。
- 嗅探：`backend/scripts/probe_upstream_topology.py`；计划：`docs/plans/2026-06-30-流量溯源干线geom重构计划.md`。
