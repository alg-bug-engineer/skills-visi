...
class Orchestrator:
    ...
    async def _diagnose(
        ...
        corridor_context = await self._corridor.build(
            session.inter_id,
            session.resolved_intersection or "",
            session.nlu,
        )
        external_evidence = await self._context_tags.build(
            session.inter_id,
            session.resolved_intersection or "",
        )
...
{conclusion}"""
    ...
    async def _finish_fast_path_diagnosis(
        self,
        session: Session,
        content: str,
        emitter: ExecutionEmitter | None,
    ) -> dict[str, Any]:
        """Complete diagnosis when an existing skill was matched on fast path."""
        assert session.matched_skill_id is not None
        skill = self._skills.get_by_id(session.matched_skill_id)
        header = "⚡ Skill 快路径命中\n\n"
        base = f"{header}{content}"
        ...
        if not skill:
            ...
            if emitter:
                await self._emit_map_sequence(
                    emitter,
                    action="confirm_bubble",
                    data={"action_type": "create", "message": "历史 Skill 不可用，是否重新固化？"},
                )
                await self._emit_map_sequence(
                    emitter,
                    action="input_dock",
            ...
            return self._build_response(
                ...
                ReplyType.DIAGNOSIS,
                (
                    f"{base}\n\n---\n历史 Skill 记录不可用，是否重新固化？"
                    "回复「是」确认，「否」结束。"
                ),
                extra={"skill_action": "awaiting_create"},
            )

        diff = self._skills.diff_with_session(skill, session)
        log_event(
            logger,
            logging.INFO,
            "skill.fast_path.diff",
            skill_id=skill.skill_id,
            has_diff=diff.has_material_diff,
            changes=diff.changes,
        )

        if not diff.has_material_diff:
            session.state = SessionState.DONE
            verified = (
                f"{base}\n\n---\n"
                "✅ **Skill 校验通过**：历史 Skill 与本次数据复核结论一致，无需更新。"
            )
            if emitter:
                await self._emit_map_sequence(
                    emitter,
                    ...
                    data={
                        ...
                        "skill_id": skill.skill_id,
                        "message": (
                            "Skill 校验通过：历史技能包与本次诊断结论一致，无需更新。"
                        ),
                    },
                )
                await self._emit_map_sequence(
            ...
            return self._build_response(
                ...
                ReplyType.TEXT,
                verified,
                extra={"skill_action": "verified", "skill_id": skill.skill_id},
            )

        session.pending_skill_action = "update"
        session.state = SessionState.AWAITING_CONFIRM
        if emitter:
            await self._emit_map_sequence(
                emitter,
                action="confirm_bubble",
                data={"action_type": "update", "message": "检测到与历史 Skill 差异，是否更新？"},
            )
            await self._emit_map_sequence(
                emitter,
                action="input_dock",
        ...
        diff_lines = "\n".join(f"- {line}" for line in diff.changes)
        update_prompt = (
            f"{base}\n\n---\n检测到与历史 Skill 的差异：\n{diff_lines}\n\n"
            "是否用本次结果**更新**已有 Skill？回复「是」更新，「否」仅作参考。"
        )
        return self._build_response(
            session,
            ReplyType.DIAGNOSIS,
