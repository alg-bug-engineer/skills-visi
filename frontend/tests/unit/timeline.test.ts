...
describe('ACT_DEFS', () => {
  it('defines nine acts with evidence → attribution → cases → plan order', () => {
    ...
    expect(ACT_DEFS[2].scene.zoom).toBe(18.8)
    expect(ACT_DEFS[3].id).toBe('act5_bottleneck')
    expect(ACT_DEFS[4].id).toBe('act6_corridor')
    expect(ACT_DEFS[4].scene.pitch).toBe(48)
    expect(ACT_DEFS[4].scene.zoom).toBe(13.8)
    expect(ACT_DEFS[5].id).toBe('act4_attribution')
    expect(ACT_DEFS[5].processTitle).toBe('归因分析')
    expect(ACT_DEFS[5].scene.zoom).toBe(18.6)
    expect(ACT_DEFS[6].id).toBe('act7_cases')
    expect(ACT_DEFS[6].processTitle).toBe('案例校验')
    expect(ACT_DEFS[6].scene.camera).toBe('hold')
    expect(ACT_DEFS[8].reveal).toBe('plan')
    expect(ACT_DEFS[8].phase).toBe('plan')
    expect(ACT_DEFS[8].scene.zoom).toBe(17.1)
