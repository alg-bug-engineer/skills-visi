...
对 40 个有数据路口 × 3 时段（120 条记录）跑真实扫描并经扫描 API 走查，验收标准 1–5 全部通过
（标准 6「关键分层/评分有单测」由 `tests/` 覆盖）：

```
[PASS] 验收1 快照可列出
[PASS] 验收2 路口含坐标 / 含 problem_band / band 过滤生效
[PASS] 验收3 详情含认知+诊断+治理
[PASS] 验收4 试点榜降序 / 仅配时可解
[PASS] 验收5 过饱和→工程可解
分层分布：{'数据不足': 36, '工程可解': 31, '配时可解': 53}  过饱和=31  试点候选=53
```

> 地图的高德可视化渲染需在浏览器（含外网访问高德）中人工核对；其数据来源（坐标、着色值、
> 分层、试点榜）已由上述 API 端到端校验。前端 `npm run build`（vue-tsc 类型检查 + 构建）与
> `npm test`（vitest 9 项）均通过。

## 运行脚本

```bash
bash scripts/scan.sh                     # 全区域扫描出快照
bash scripts/serve.sh                    # 起扫描 API（8100）
cd frontend && npm install && npm run dev # 起前端（5570，代理 /api → 8100）
```
